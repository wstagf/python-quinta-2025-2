...
try:
    pesoemTexto = input("Digite seu peso em kg: ")

    if(pesoemTexto.find(",") > 0):
        pesoemTexto = pesoemTexto.replace(",", ".")


    
    peso = float(pesoemTexto)
    
    if(peso <= 0):
        raise ValueError("Peso deve ser maior que zero.")


    alturaEmTexto = input("Digite seu altura em metros: ")

    if(alturaEmTexto.find(",") > 0):
        alturaEmTexto = alturaEmTexto.replace(",", ".")

    altura = float(alturaEmTexto)
